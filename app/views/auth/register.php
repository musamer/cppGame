...
<div class="w-full max-w-md mx-auto mt-12 bg-cardDark border border-[#30363d] rounded-2xl p-8 shadow-2xl relative overflow-hidden">
    ...
    <div class="relative z-10">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-white mb-2"><?= __('register_welcome') ?></h1>
            <p class="text-textSecondary"><?= __('register_subtitle') ?></p>
        </div>

        <form action="<?= URLROOT; ?>/auth/register" method="POST">
            <div class="mb-4">
                <label class="block text-textSecondary font-bold mb-2"><?= __('username_label') ?></label>
            ...
            <div class="mb-6">
                <label class="block text-textSecondary font-bold mb-2"><?= __('confirm_password') ?></label>
            ...
            <button type="submit" class="w-full bg-brandPurple hover:bg-brandPurpleHover text-white font-bold py-3 px-4 rounded-xl transition-colors shadow-lg shadow-brandPurple/20 mb-4"><?= __('start_journey') ?></button>

            <p class="text-center text-textSecondary"><?= __('already_knight') ?> <a href="<?= URLROOT; ?>/auth/login" class="text-brandPurple hover:text-white transition-colors"><?= __('login_here') ?></a></p>
        </form>
    </div>
</div>
